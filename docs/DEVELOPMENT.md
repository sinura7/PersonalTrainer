...
|---|---|---|---|
| Static gate: `PT_STATIC_ONLY=1 tools/preflight.sh` | 25 source checkers (counted 25 September 2026), 10 fixture proofs, ratchets in `tools/checker-baselines.toml`, syntax check | Any machine with Python 3.11+ (`tomllib`) and Java 17. The syntax check needs the Kotlin compiler jar, which the gate fetches through Gradle when it is missing; where Gradle cannot run (no SDK), set `PT_ALLOW_NO_COMPILER=1` and the check is skipped with a warning instead of failing the gate | every commit |
...
| Build + lint: `./gradlew assembleDebug lintDebug` | The APK compiles; so does the release, because `assembleDebug` depends on `assembleRelease` (R8, resource shrinking, lint-vital — what `release.yml` runs; unsigned there, since only a release task named in full is signed); no new lint issues past `app/lint-baseline.xml` | same | merge into `trunk` |
...
copy, and the release build `assembleDebug` now carries (X7), while
tightening an inner loop in the IDE, and is not a way through the gate.
Android Studio's Run builds through `assembleDebug`, so it pays for the
release build too unless that flag is set.
...
preflight runs, unit tests, blocking lint, a debug APK artifact and the
unsigned release build behind it, the instrumented-sources compile, and
the non-blocking hosted emulator lane),
...
[SETUP.md](../SETUP.md). The gate builds one anyway (`assembleDebug` depends on it), only to
prove the release still builds. It is unsigned even where `keystore.properties` exists, because
the build signs a release only when a release task is asked for by name; nobody installs it. When R8 stops on a missing
class it writes the rule it wants to `app/build/outputs/mapping/release/missing_rules.txt`.
Read why before copying it into `app/proguard-rules.pro`: a `-dontwarn` is right only when
the code that names the class survives its absence, as slf4j's binder lookup does.
