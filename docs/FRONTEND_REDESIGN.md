...
| 12e | R3 | Temper Debug updater: `MainActivity` handles `STATUS_PENDING_USER_ACTION` (RM-1); staged APKs deleted from cache (RM-6) | Q | Done — #436: Android's answer goes to a Temper Debug-only screen that is not exported (not `MainActivity`, which any app can open), which opens Android's install sheet and tells the banner a refusal; one download is kept at a time and none once installed; rides the check-only drop |
