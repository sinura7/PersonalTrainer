...
| 9–12 | W2a–d | Dead code and lower token ceilings · shared rest commands, atomic `adjust` · coach/picker performance · save/undo and session-state extraction | Q | W2b-1 done — #403; W2b-1b done — #405; W2b-1c (a finish cannot cancel the next rest's save; a rest started as the last one finishes still reaches the shade) in review. Then, order of 23 September amended 24 September: after T1c-2, W2a (with the rest-length sheet honouring reduce motion), W2b-2 (shared rest commands), W2c, W2d-1–3 (undo, save, timed work), then a check-only drop |
