...
| 12d2 | Rest alarm | The rest-alarm packet: the "Rest alerts" sentence asks once (RT-2); the rest countdown readable in the shade and on the lock screen under the light theme (RT-1); no alarm armed for a rest whose row is not on disk, and none after a failed save (the two quirks, RT-4 and RT-5) | V | In three PRs, run beside W2d-3a to W2e in another session (owner decision of 26 September): RT-2 done — #433 (the sentence is answered once on the phone, Continue or Not now; the compact row on the workout and the rest page stays the way back); RT-1 and the two quirks pending |
