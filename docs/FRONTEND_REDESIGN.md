...
| 12f | R4 | Reminder follow-ups found by R2-6: with nothing live, a Start tapped on a screen pushed over Home (the workout summary) goes nowhere, then starts the session by itself when that screen is left; a planned day's reminders cancelled once its session opens, from any start, so its Move and Skip cannot act on the running day | V | Done — #R4PR: with nothing live, a Start tapped over a screen left open on Home (a workout's summary, a finished session's page) opens the planned session at once; over an activity being logged or a routine being edited, which ask before they are left, it stays there and says why, the reminder stays in the shade, and nothing starts when that screen is left. Once a planned day's session opens, from any start, its reminder leaves the shade; one due while the session runs is not shown, and Snooze, Move and Skip on one still showing leave the day alone. Its later reminders are not cancelled: after a discard they still come. Rides the check-only drop |
