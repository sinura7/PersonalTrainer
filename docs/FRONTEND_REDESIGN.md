...
| 12c | R1 | Data safety: "Generate a week" confirms and keeps the current block (UI-3); the restore guard counts goals, templates and planner rules (BK-3); Drive sign-out disarms auto-backup and forgets the sealed password (BK-1); the after-workout copy survives leaving the summary (BK-4); a `corruptionHandler` on `user_settings` and the chooser drawn under, not over, the retry screen (DB-2, L-3) | V | In progress, in five PRs: R1-1 done — #419 (Drive sign-out switches automatic backup off and forgets its password, and asks first); R1-2 done — #420 (the restore guard counts goals, templates and the weekly plan); R1-3 done — #421 (the after-workout copy belongs to the app; one Drive backup at a time); R1-4 ("Generate a week") and R1-5 (the settings file) next |
