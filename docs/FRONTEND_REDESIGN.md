...
| 12b | X7 | Release lane: `-dontwarn org.slf4j.impl.StaticLoggerBinder` in `proguard-rules.pro` (BR-1); `packages: ''` in `release.yml` (BR-2); the swallowed upload failure (BR-7); an unsigned `assembleRelease` in the local gate so the release cannot rot silently again | Q | In review (owner decision of 25 September, [audit X6](design-audit/2026-09-25/AUDIT.md)): the release builds again; `release.yml`'s SDK and publish steps fixed; `assembleDebug` builds the release, and `tools/check-release-lane.py` holds all three |
