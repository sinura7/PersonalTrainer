...
| 12f | R4 | Reminder follow-ups found by R2-6: with nothing live, a Start tapped on a screen pushed over Home (the workout summary) goes nowhere, then starts the session by itself when that screen is left; a planned day's reminders cancelled once its session opens, from any start, so its Move and Skip cannot act on the running day | V | Pending — owner decision of 26 September: after R3, before the check-only drop |
