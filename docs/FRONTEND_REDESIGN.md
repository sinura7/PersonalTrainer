...
| 6 | T1 | Workout test triage: source-string assertions that pin removable code become rendered/semantic checks | Q | T1a done — #388; T1b done — #392; T1c-1 done — #404; T1c-2 before W2a |
...
| 9–12 | W2a–d | Dead code and lower token ceilings · shared rest commands, atomic `adjust` · coach/picker performance · save/undo and session-state extraction | Q | W2b-1 done — #403; W2b-1b (a stop names its rest; a skipped rest never says "Rest done" or comes back; a finished rest stays done under a late Skip) in review. Then, order of 23 September amended 24 September: W2b-1c (a finish cannot cancel a new rest's save); after T1c-2, W2a (with the rest-length sheet honouring reduce motion), W2b-2 (shared rest commands), W2c, W2d-1–3 (undo, save, timed work), then a check-only drop |
