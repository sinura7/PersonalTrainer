...
| 9–12 | W2a–d | Dead code and lower token ceilings · shared rest commands, atomic `adjust` · coach/picker performance · save/undo and session-state extraction | Q | W2b-1 done — #403; W2b-1b done — #405; W2b-1c done — #406; W2b-1d done — #407; W2a done — #409 (dead code out, four token ceilings down, the rest-length sheet honouring reduce motion); W2b-2 done — #411 (the Log and the rest page share one set of rest commands and one hint loader; no behaviour change); W2b-3 done — #412 (the lock screen's and rest page's Skip name their rest; the dock's Skip is unchanged); W2b-4 done — #413 (the rest page's next-set line is the Log's, shown only where the Log shows it); W2c done — #414 (the coach is asked again only when something it reads changes, not on every tap or every second of rest; the picker sorts only while it is open; the lift it suggests follows edits to that lift; no behaviour change); W2d-1 done — #417 (the floor's undo queue moves out of the workout ViewModel into `FloorUndoOffers`, pinned first by 16 new tests; no behaviour change; built before the 25 September order landed and merged ahead of R1 and R2 on the owner's word of 25 September); T3 done — #430 (tests only: the flaky notes test waits for what it reads, and new tests hold the notes' saves, among them leaving while a lift loads and a phone killed mid-load); W2d-2 done — #431 (the floor's set save moves into `FloorSetSaves`, pinned first by 13 new tests; no behaviour change); N1 done — #432 (a workout reopened while its lift was still loading keeps its newest notes and its own lift's numbers; notes are kept once per workout; visible, on the drop 105 checklist); W2d-3a in review (the floor's hold countdown and set clock move into `FloorWorkClocks`, pinned first by 25 new tests; no behaviour change). Then, order of 25 September (audit X6, owner decision; refined by the owner later that day): X7 and R1 first (rows 12b, 12c), R2 (12d), N1 (above), W2d-2 (save), W2d-3a (timed work), W2d-3b (notes), W2e (a lighter week or a unit switch made while a lift is open reaches its pre-filled numbers; typed numbers stay; visible), the rest-alarm packet (the two quirks, the rest-alerts gate that asks once, the light-theme notification colours), R3 (12e), R4 (12f), then a check-only drop. Audit X6's AR-3 and AR-4 (the invalidation fan-out per logged set and the eager `stateIn` on the floor, which W2c left) leave W2d for a later packet that needs owner decisions |
...
| 12d | R2 | Crash and coach: the four Home feeds through `observeHealth` and a handler on the insights scope (DB-1, AR-1); History's activity read guarded (UI-17); the bar's finish/discard epilogue guarded (UI-12); diagnostics capture installed before the pre-migration copy (AR-2); hold lifts excluded from rep coaching (DM-1); Finish blocked or warned while a set edit is open (UI-2 B11); a reminder tap that neither ejects a live workout nor consumes a blocked start (UI-1 B9a) | V | Done, in six PRs: R2-1 done — #424 (Home and History keep what they last read instead of closing the app when a read fails; History's behind line offers Retry); R2-2 done — #425 (a finish, discard or start stands when the write after it fails; the bar opens the summary instead of crashing); R2-3 done — #426 (diagnostics start before the pre-migration copy, so a copy that fails while being written reaches them); R2-4 done — #427 (the coach no longer talks in reps about planks and other holds); R2-5 done — #428 (Finish warns while a logged set's correction is open, and the bar's Finish waits for it); R2-6 done — #429 (a reminder tapped during a workout leaves you where you are and waits; it is used only once its session opens) |
| 12d2 | Rest alarm | The rest-alarm packet: the "Rest alerts" sentence asks once (RT-2); the rest countdown readable in the shade and on the lock screen under the light theme (RT-1); no alarm armed for a rest whose row is not on disk, and none after a failed save (the two quirks, RT-4 and RT-5) | V | Done, in three PRs, run beside W2d-3a to W2e in another session (owner decision of 26 September): RT-2 done — #433 (the sentence is answered once on the phone (Continue, Not now, or closing it); the compact row on the workout and the rest page stays the way back); RT-1 done — #434 (the countdown in the shade and on the lock screen takes the phone's notification text colours, so it reads on a light theme); RT-4 and RT-5 done — #435 (a rest's wakeup is armed only after its own row is on disk, and only for that rest) |
| 12e | R3 | Temper Debug updater: the install's `STATUS_PENDING_USER_ACTION` answer opens Android's install sheet (RM-1); staged APKs deleted from cache (RM-6) | Q | Done — #436: Android's answer goes to a Temper Debug-only receiver that is not exported (not `MainActivity`, which any app can open); the app opens Android's install sheet while it is in front, at once or when the owner comes back, and the banner shows a refusal; one download is kept at a time and none once the install ends; rides the check-only drop |
| 12f | R4 | Reminder follow-ups found by R2-6: with nothing live, a Start tapped on a screen pushed over Home (the workout summary) goes nowhere, then starts the session by itself when that screen is left; a planned day's reminders cancelled once its session opens, from any start, so its Move and Skip cannot act on the running day | V | Done — #437: with nothing live, a Start tapped over a screen left open on Home (a workout's summary, a finished session's page) opens the planned session at once; over an activity being logged or a routine being edited, which ask before they are left, it stays there and says why, the reminder stays in the shade, and nothing starts when that screen is left. Once a planned day's session opens, from any start, its reminder leaves the shade (a mixed day's when its composer opens); one due while the session runs is not shown, and Snooze, Move and Skip on one still showing leave the day alone. A mixed day's composer is not a live session, so a reminder due while it is open still shows, with its Move and Skip (recorded). Its later reminders are not cancelled: after a discard they still come. Rides the check-only drop |
