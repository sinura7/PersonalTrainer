...
| UX06-AC04 | P0 | F10 | All shared filter callers are covered, including composer, live cardio and routine targets. | Executed for the domain rules; caller wiring changed in composer, live cardio, custom rest, routine card — Compose-level check pending (no SDK here) |
