...
| UX06-AC04 | P0 | F10 | All shared filter callers are covered, including composer, live cardio and routine targets. | Executed for the domain rules; caller wiring changed in composer, live cardio, custom rest, routine card and the custom week — every caller now type-checks against the widened signature (`tools/compile-check.sh` compose stage, 378 files) — Compose-level render check pending (no SDK here) |
