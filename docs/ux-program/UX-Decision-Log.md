...
| D16 | ADR-021 wording (new) | The UX04 implementation appended one paragraph to ADR-021 item 7 describing Save's truthfulness (leaves only when its writes landed; Back asks). It does not reverse the decision, but it edits a signed record. **Owner to accept or move the paragraph to this register.** | Owner acknowledgement requested |
...
| D19 | An Android compiler for this environment (new) | Nothing here could type-check the Android side, so a signature widened in `e5d2956` left two stale callers and the branch was pushed unbuildable (handoff §6.1). `tools/compile-check.sh` now runs the real Kotlin 2.0.21 compiler over `app/src/main/java` using real library declarations from Maven Central (Robolectric's framework jar for `android.*`, JetBrains Compose Multiplatform 1.8.2 plus the app's own Compose compiler plugin for `androidx.compose.*`) and declaration-only stubs only where nothing is fetchable (Room, DataStore, WorkManager, lifecycle, GMS). Google's Maven is unreachable and the proxy was not bypassed. Alternatives: accept the gap and review by eye (rejected — three reviewers, twenty checkers and the JVM lane all missed this defect); vendor a full Android SDK (not available); substitute nearby real builds and record exactly what that cannot see (chosen). Deliberately **not** in `preflight.sh`: the first run fetches ~186 MB. `--explain` prints the soundness ledger. Alongside it, `check-lambda-arity.py` is the offline guard for the same defect class, and four existing checkers were widened to read `androidTest`, `debug` and `sharedTest` — source sets no tool in the repo had ever opened, which is where the second stale caller sat. Every widening was negative-controlled. | Implemented; a green here is not `assembleDebug` |
