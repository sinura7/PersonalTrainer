...
`#174` (coroutines 1.11.0 — the leak was `kotlinx-coroutines-android`,
which the kotlin group still matched after core and test were ignored),
and `#176` (android-all-instrumented 17; J3 stays on the API-35 jar).
Leave `#173` (setup-gradle 6.3.0; Actions is not the test lane) and
`#175` (Robolectric 4.16.1 patch) unless a dedicated JVM-gated packet
takes them with a ledger update.
