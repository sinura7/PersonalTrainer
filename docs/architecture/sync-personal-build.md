...
- **Authority:** [ADR-004](ADR-004-offline-core-and-entitlements.md), [ADR-031](ADR-031-trusted-server-sync-lane.md) (trusted-server personal lane, not E2EE; amends [ADR-009](ADR-009-backup-privacy-sync.md) §14–15; the bar for resuming is its decision 3)
- **Paused (22 September 2026, whole-app audit packet S0a):** `domain/AccountSyncGate.SYNC_PAUSED` stops every pass. `PausableSyncScheduler` wraps the one WorkManager scheduler, and `SyncCoordinator(paused)` makes `requestSync` and `runPass` no-ops (a job persisted by an older build runs, does nothing, and reports success). Authoring hooks still write `sync_outbox` while signed in. Reason at the time: pull applied rows with Room `REPLACE` / delete+insert, and FK cascades then dropped child rows (activity sets, routine lifts, `workout_sessions.routineId`). Packet S0b fixed the pull (below); the pause stays until packet S1 meets [ADR-031](ADR-031-trusted-server-sync-lane.md) decision 3 and the owner says yes. Everything below describes sync when it runs.
- **Pull writes in place (packet S0b):** a pulled update changes that row and nothing under it. Activity sessions, blocks, sets, intervals, templates and routines go through Room `@Upsert` (`ActivityDao.upsert*InPlace`, `RoutineDao.upsertRoutineInPlace`), which updates an existing row instead of deleting it first; a set or interval already on the phone no longer throws and stalls the tables after it. Pulled deletes still cascade, as they should. A custom-lift delete that a row on this phone still references (RESTRICT keys on `set_logs`, `session_exercises`, `routine_exercises`) is held and retried once routine lifts are pulled, because the other phone deleted the routine lift first; a lift this phone's own history uses is kept, with its muscle credits. `SyncPullInPlaceTest` covers each case.
...
- **First install save posture ([ADR-028](ADR-028-save-posture-and-account-sync-target.md)):** After that intro, a one-time chooser offers Temper Account (Settings → Account) or continue on this phone, with an optional jump to Settings → Backup for Google Drive. Persisted in DataStore (`save_posture`, `save_posture_chosen`). The chooser blocks touches, Back and TalkBack from reaching Home beneath it, and scrolls; after Account or Drive the permission walk waits until a tab other than Settings is on screen ([ADR-028](ADR-028-save-posture-and-account-sync-target.md) §2, audit Q1). Settings → **How you save** can change the posture later. Upgrades with prior permissions, plan setup, Drive, or Account skip the chooser.
