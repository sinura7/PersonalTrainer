...
|---|---|---|
| BR-1 (B14) | The signed gym-floor version cannot be built at all: a library added on 21 September trips the code shrinker (`org.slf4j.impl.StaticLoggerBinder`), and nobody has built the release flavour since. Temper Debug is unshrunk and unaffected. One line fixes it. | X7 |
| BR-2 (B14) | The release robot has a second, independent break in its very first step (the retired `tools` SDK package), already fixed in the other two workflows. | X7 |
...
| # | Packet | Scope | Kind |
|---|---|---|---|
| 1 | **X7** release lane | `-dontwarn org.slf4j.impl.StaticLoggerBinder` (BR-1); `packages: ''` in `release.yml` (BR-2); the swallowed upload failure (BR-7); an unsigned `assembleRelease` in the local gate so the release cannot rot silently again | Q |
