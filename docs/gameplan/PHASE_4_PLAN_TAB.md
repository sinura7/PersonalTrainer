...
Line numbers throughout this packet are from audit commit `2212628` on `claude/app-hierarchy-navigation-cjzigo`. Phases 0, 2, 1 and 3 have merged since — in that order (§4 "Execution order"; Phase 1 ships as ONE PR covering both the 1A and 1B packets, so "Phase 1a" below names that packet's work, not a separate merge); lines will have drifted — especially in `AppNav.kt`, which Phase 1a's LiveSessionBar edits. Re-anchor every citation by content (grep the quoted identifier) before editing.

1. `docs/gameplan/PROTOCOL.md` — branch/PR/sign-off rules this phase runs under. The only other gameplan file you need.
2. `docs/gameplan/SCHEDULE_SEMANTICS.md` (Phase 0 deliverable — `docs/gameplan/PHASE_0_DECISIONS.md` WI-5 creates it under exactly that name and is the only place that names it; PROTOCOL.md carries no file index, so do not go looking for one) — the owner-signed slot semantics and five worked examples. §4 of this packet restates them; **the signed doc wins on any conflict.**
...
**Execution order (phase numbers are identifiers, not sequence).** The order is 0 → 2 → 1 → 3 → **4** → 5 → 6a → 6b → 7 → 8. Four phases are merged before you start: **Phase 0** (decisions; the schedule-slot semantics signed), **Phase 2** (test substrate: `tools/preflight.sh` and the Robolectric JVM lane), **Phase 1** (session hygiene — one PR from branch `claude/phase-1-session-hygiene` covering both the 1A and 1B packets), **Phase 3** (schema v2). Tests: Phase 0 merged ⇔ `grep -c "Signed:" docs/ROADMAP.md` returns greater than 0 — `docs/gameplan/PROTOCOL.md` is committed on every branch, so its presence proves NOTHING; Phase 2 merged ⇔ `tools/preflight.sh` exists; Phase 1 merged ⇔ the `claude/phase-1-session-hygiene` merge is in trunk's `git log`; Phase 3 merged ⇔ the first bullet below.
...
**Mandatory first commit: the re-baseline report (D-G, PROTOCOL.md §6).** Every count, line number, and repo-state assertion in this packet is a baseline as of audit commit `2212628`, not an oracle — four phases have landed since. Your FIRST commit on `claude/phase-4-plan-tab` is a docs/PR-body re-baseline report, before any work item. It states: the current trunk tip (`git log --oneline -1`) and which phases merged since `2212628`; the actual domain-test count and test-class count from a real run, not from this packet; and every packet literal that has drifted with its verified current value (at minimum the `AppNav.kt`, `ScheduleScreen.kt`, `ScheduleViewModel.kt`, `HomeScreen.kt`, `SettingsScreen.kt`, `TrainingInsightsSource.kt` and `WeeklySchedulePlanner.kt` line cites, and the five WI-5 proof greps). Drift fully explained by Phases 0/2/1/3 or by the game plan's own commits is EXPECTED — record it, adopt the new value, proceed. Stop only on a mismatch with no such explanation.
...
- **Create `ui/home/ThisWeekCard.kt`:** extract `ThisWeekHomeCard` (`ScheduleScreen.kt:412-465`) as `ThisWeekCard(day, nextDay, loggedToday, onOpenPlan, onSuggestWeek, onPrimary)`. Drop the `thinHistory` param and its caption (derived weeks set it false). **Expect the in-progress branches to be present, and delete them here.** Phase 1a did NOT remove them: it *neutered* them by passing `inProgress = false` from `HomeScreen`, because that packet forbids editing `ScheduleScreen.kt` — verified still live at `ScheduleScreen.kt:424-441` (`inProgress -> "In progress"` in the kicker `when`, `inProgress -> "Resume workout"` in the action `when`). Drop the `inProgress` parameter and both branches during the extraction; the card must carry no live-session affordance (LiveSessionBar is the only one, §3).
- **Empty-week state on `ThisWeekCard` — the cliff mitigation.** When `day == null && nextDay == null`: headline `"No plan yet"`, caption `"Pin your week in Plan, or let the app propose one."`, **primary action `"Suggest a week"`** → `onSuggestWeek`; `"Start a workout"` is demoted to a secondary `TextButton` under it → `onPrimary` (the existing free-workout path, unchanged). Card body tap stays `onOpenPlan`. Rationale, recorded: this phase deliberately deletes the auto-generated ghost week, so the empty state is what every owner sees on the first launch after the update — recovery from it must be **one tap from Home**, not a tab hunt followed by a hunt for the button.
  - **Mechanism for `onSuggestWeek` (deep-link with the suggestion pre-armed).** One-shot flag, app-scoped, consumed exactly once — the same idiom and the same reason as the nav-StateFlow documented at `ScheduleViewModel.kt:88-102` (a lambda captured into a coroutine dies with the composition; a StateFlow outlives it): `AppContainer` (`:22-70`) gains `val pendingWeekSuggestion = MutableStateFlow(false)`. `HomeViewModel` gains `fun requestWeekSuggestion() { container.pendingWeekSuggestion.value = true }`. `HomeScreen` wires `onSuggestWeek = { viewModel.requestWeekSuggestion(); onOpenPlan() }` — **no new `AppNav` callback**: `onOpenPlan` already lands on the Plan tab via `goToTab`. `PlanViewModel.init` collects `container.pendingWeekSuggestion` in `viewModelScope`; on `true` it writes `false` back first (consumed once, survives an Activity recreation between the tap and arrival) and then calls `suggestFills()`, so the Plan tab opens with the proposal preview already on the strip. Nothing is persisted by the deep link — "Accept fills" is still the only writer.
...
12. The empty-week recovery (do this before you pin anything, or after unpinning the week): Home's hero reads **"No plan yet"** and its primary button is **"Suggest a week"** — tapping it lands you on the Plan tab with a proposed week already previewed on the strip, one "Accept fills" away. "Start a workout" is still there underneath as the smaller secondary action.
13. Reply on the PR with pass/fail per step. The phase closes on your sign-off.
...
- **Owner:** 0.5-1 day — the 13-step device pass plus PR review. The PR is large but mechanically partitioned: review WI-1/WI-3 (behavior) carefully; WI-4/WI-5 (UI/moves) can be reviewed by screenshot and checklist.
...
5. A plain-language note of the one behavior change the owner will feel immediately: **the auto-generated ghost week is gone** — Home and Plan show only pinned/accepted sessions, and the first thing to do after updating is pin a week or tap "Suggest a week" — from Home's empty hero or from the Plan tab — then "Accept fills".
...
10. The next phase, named: **Phase 5 — Honest heat & coach** (`docs/gameplan/PHASE_5_HEAT_COACH.md`). Execution order is 0 → 2 → 1 → 3 → **4** → 5 → 6a → 6b → 7 → 8 (phase numbers are identifiers, not sequence). Hand Phase 5 the shipped Plan tab, the six-source `TrainingInsightsSource`, and the re-baseline report from this phase's first commit — Phase 5's own citations into `TrainingInsightsSource.kt`, `HomeScreen.kt` and `WeeklySchedulePlanner.kt` land on files this phase moved.
