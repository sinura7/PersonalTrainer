...
>
> **Execution order (phase numbers are identifiers, not sequence).** The order is 0 → 2 → 1 → 3 → 4 → 5 → 6a → 6b → **7** → 8. Everything except Phase 8 is merged before you start: **Phase 0** (decisions), **Phase 2** (test substrate), **Phase 1** (session hygiene — ONE PR from `claude/phase-1-session-hygiene`, both the 1A and 1B packets), **Phase 3** (schema v2), **Phase 4** (Plan tab), **Phase 5** (heat & coach), **Phase 6a/6b** (tabs, Body, Home). Verify Phase 0 with `grep -c "Signed:" docs/ROADMAP.md` returning greater than 0 — `docs/gameplan/PROTOCOL.md` is committed on every branch, so its presence proves NOTHING. Phase 8 (imagery) is explicitly OPTIONAL and nothing here depends on it: `imageKey` stays null and renders composed whether or not it is ever executed.
>
> **Mandatory first commit: the re-baseline report (D-G, PROTOCOL.md §6).** Every count, line number, file path, and repo-state assertion in this packet is a baseline as of audit commit `2212628`, not an oracle — eight phase merges have landed since, and Phases 5, 6a and 6b rewrote several of the files cited in §2. Your FIRST commit on `claude/phase-7-catalog` is a docs/PR-body re-baseline report, before any work item: the current trunk tip (`git log --oneline -1`) and which phases merged since `2212628`; the actual domain-test count and test-class count from a real run, not from this packet; and every packet literal that has drifted, with its verified current value — at minimum Phase 3's shipped `CATALOG_VERSION` (this packet's N), the shipped batch-1 count and `movementKey` values (§4.14), the shipped `loadType` of every batch-1 bodyweight-family lift, and every §2 line cite. Drift fully explained by a merged prior phase or by the game plan's own commits is EXPECTED — record it, adopt the new value, proceed. Stop only on a mismatch with no such explanation (a batch-1 `movementKey` that is not the §4.14 family key is exactly such a mismatch).
...
3. **`muscleKey` vocabulary (tail):** a `muscleKey` is exactly `CanonicalMuscle.name.lowercase()` — nothing else is legal, and this is the same vocabulary Phase 3 shipped for batch 1 (its S5). The ten keys: `chest, back, shoulders, biceps, triceps, quadriceps, hamstrings, glutes, calves, core`. **`quads` is NOT a `muscleKey`** — it is an alias that normalizes to QUADRICEPS (CanonicalMuscle.kt:122-129, :60) and would slip past a normalization-only check while being wrong in the column; the invariant test in WI-1 asserts exact equality, so writing it fails the build. Aliases belong in `searchTerms` and in free display text (`muscleGroup`), never in `exercise_muscles.muscleKey`.
...
14. **movementKey vocabulary + family labels** (the whole 98): `squat`→Squat, `lunge`→Lunge, `step-up`→Step-Up, `leg-press`→Leg Press, `leg-extension`→Leg Extension, `deadlift`→Deadlift, `romanian-deadlift`→Romanian Deadlift, `good-morning`→Good Morning, `back-extension`→Back Extension, `kettlebell-swing`→Swing, `hip-thrust`→Hip Thrust, `hip-abduction`→Hip Abduction, `glute-kickback`→Kickback, `pull-through`→Pull-Through, `leg-curl`→Leg Curl, `nordic-curl`→Nordic Curl, `calf-raise`→Calf Raise, `bench-press`→Bench Press, `chest-fly`→Chest Fly, `push-up`→Push-Up, `dip`→Dip, `overhead-press`→Overhead Press, `lateral-raise`→Lateral Raise, `rear-delt`→Rear Delt, `row`→Row, `pulldown`→Pulldown, `pull-up`→Pull-Up, `pullover`→Pullover, `shrug`→Shrug, `curl`→Curl, `triceps-extension`→Triceps Extension, `plank`→Plank, `crunch`→Crunch, `sit-up`→Sit-Up, `leg-raise`→Leg Raise, `rollout`→Rollout, `twist`→Twist, `dead-bug`→Dead Bug, `carry`→Carry.
    Batch-1 keys — **Phase 3 ships exactly these** (its S6: family keys, closed set of 23 families, explicitly "never re-keyed later"), so the tail above simply extends the same vocabulary and there is nothing to align. Do NOT re-key either side: verify at execution with a grep of the shipped catalog source for `movementKey` and diff it against this list; if anything mismatches, that is an unexplained drift under PROTOCOL.md §6 — report it to the owner and stop rather than silently re-keying, because §7's family tests (`LibraryGroupingTest`: the `bench-press` family has 8 members — 4 from batch 1, 4 from batch 2) ride on these exact values. The list, as verification reference: Back/Front/Goblet Squat→`squat`; Bulgarian Split Squat, Walking Lunge→`lunge`; Leg Press→`leg-press`; Leg Extension→`leg-extension`; Conventional/Trap Bar Deadlift→`deadlift`; Romanian Deadlift→`romanian-deadlift`; Hip Thrust→`hip-thrust`; Leg Curl→`leg-curl`; Standing Calf Raise→`calf-raise`; Barbell/Incline/Dumbbell Bench Press + Close-Grip Bench Press→`bench-press`; Push-Up→`push-up`; Chest Fly→`chest-fly`; Overhead Press, Seated Dumbbell Press→`overhead-press`; Lateral Raise→`lateral-raise`; Face Pull→`rear-delt`; Barbell/Pendlay/One-Arm Dumbbell Row, Seated Cable Row→`row`; Lat Pulldown→`pulldown`; Pull-Up, Chin-Up→`pull-up`; Barbell/Dumbbell Curl→`curl`; Tricep Pushdown, Skull Crusher→`triceps-extension`; Plank→`plank`; Hanging Leg Raise→`leg-raise`; Cable Crunch→`crunch`.
...
| Push Press | ex-push-press | overhead-press | BARBELL | EXTERNAL | shoulders | triceps@0.5, quadriceps@0.25 | — | 375 |
...
| Name | id | movementKey | equip | load | primary | sec | terms | rank |
|---|---|---|---|---|---|---|---|---|
| Hack Squat | ex-hack-squat | squat | MACHINE | EXTERNAL | quadriceps | glutes@0.5 | — | 500 |
| Smith Machine Squat | ex-smith-machine-squat | squat | SMITH | EXTERNAL | quadriceps | glutes@0.5 | smith squat | 505 |
| Reverse Lunge | ex-reverse-lunge | lunge | DUMBBELL | EXTERNAL | quadriceps | glutes@0.5, hamstrings@0.25 | — | 510 |
| Dumbbell Step-Up | ex-dumbbell-step-up | step-up | DUMBBELL | EXTERNAL | quadriceps | glutes@0.5 | step up | 515 |
| Bodyweight Squat | ex-bodyweight-squat | squat | BODYWEIGHT | BODYWEIGHT | quadriceps | glutes@0.5 | air squat | 520 |
| Sumo Deadlift | ex-sumo-deadlift | deadlift | BARBELL | EXTERNAL | glutes | back@0.5, quadriceps@0.5 | sumo | 525 |
...
**Gate output comes from the owner's machine, never from CI** (D-F): CI has never executed on this repository — the account is under a billing block — so paste the `./gradlew testDebugUnitTest` and `./gradlew assembleDebug` output from the owner's machine into the PR as the gate evidence. CI green on the `claude/**` trigger is an ADDITIONAL check once the owner's standing (non-gating) billing errand is done; a phase never waits on it. Review artifacts `docs/catalog/batch-2-review.md` and `docs/catalog/batch-3-review.md` committed and regenerated from the final catalog (invariant sections all passing).
...
3. The reconciliation note against Phase 3's shipped substrate: actual `CATALOG_VERSION` values used, any identifier/shape differences from this packet's assumptions (§4.1), and whether batch-1 movementKeys matched §4.14 (Phase 3 ships them by contract; a mismatch is reported to the owner and resolved by decision, never by silently re-keying either side — the family tests depend on this vocabulary). Include the shipped loadType of every batch-1 bodyweight-family lift (Pull-Up, Chin-Up, Push-Up, Plank, Hanging Leg Raise) since §8 steps 6-7 read off them.
...
8. **Recorded follow-up, owner decision required — bodyweight is never stored, so bodyweight lifts log wrong.** This phase ships 20+ `BODYWEIGHT` / `BODYWEIGHT_PLUS` rows (Push-Up, Dip, Pull-Up, Bodyweight Squat, Nordic Ham Curl, Side Plank, Dead Bug…) and the rep-progression copy that goes with them (§4.11), but the app stores the owner's bodyweight nowhere. Consequences, stated plainly so they are not discovered on the device: a weighted dip logged as `+20 kg` computes volume and e1RM from the **added load alone**, and a bodyweight-only set logs **0 kg of volume** — so these lifts under-count in every volume-based surface, and Phase 5's set-count bands are the only place they read correctly. `DESIGN_AUDIT.md:443` N-06 (P2) already calls for a gym profile carrying bodyweight and available plates. Put the gap on the PR with the two dispositions named, for the owner to sign:
   - **A — owned follow-up (recommended default).** A device-local `bodyweight_kg` preference in `PreferencesRepository`, exactly the shape of this phase's `library_collision_dismissed_ids` dismissal pref (DataStore, single key, one Settings row), read by the volume/e1RM math for `BODYWEIGHT` and `BODYWEIGHT_PLUS` lifts (effective load = bodyweight + added). It is a small phase of its own, not a smuggled-in extra: it changes computed history for existing sessions, which is a decision, not a refactor.
   - **B — explicit non-goal.** Bodyweight lifts are counted by sets and reps only, forever; volume and e1RM stay blank (not zero) for `BODYWEIGHT` rows, and the hand-back says so in one line the owner can re-read later.
   Ship this phase either way — nothing here is blocked by the answer. What is not acceptable is shipping the rows and the copy while leaving the arithmetic unexplained.
9. The next phase, named: **Phase 8 — Imagery** (`docs/gameplan/PHASE_8_IMAGERY.md`), which is **explicitly OPTIONAL and last**. Nothing depends on it: the `imageKey` hook shipped in Phase 3, `null` means "compose the thumb", and that is a complete shipping state. Hand it the reworked `ExerciseRow`/picker signatures, the shipped `Equipment` enum, and this phase's re-baseline report; deferring it indefinitely costs the owner nothing.
