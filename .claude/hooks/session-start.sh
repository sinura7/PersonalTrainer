...
# project could not even configure. That is why tools/compile-check.sh exists: it type-checks
# the Android sources with substitute libraries because the real build was impossible. On
# 10 September 2026 dl.google.com started answering, the SDK was installed by hand, and
# `./gradlew assembleDebug` built a real APK in 5m24s while `testDebugUnitTest` ran 1,946
# tests. This script is that afternoon's work made repeatable.
#
...
# plus tools/compile-check.sh remain the fallback they have always been. A session that cannot
# reach Google must still be a working session.
set -euo pipefail
...
# compileSdk is 36 (app/build.gradle.kts). AGP 8.9.2's default build-tools is 35.0.0 — that is
# what the real build resolved to, so it is what gets installed; accepting the licences also
# lets AGP fetch a different one itself if that default ever moves.
PLATFORM="platforms;android-36"
BUILD_TOOLS="build-tools;35.0.0"
CLT_ZIP="commandlinetools-linux-13114758_latest.zip"

if [ -d "$SDK/platforms/android-36" ] && [ -d "$SDK/build-tools/35.0.0" ] && [ -x "$SDK/platform-tools/adb" ]; then
...
if ! curl -fsS -o /dev/null --max-time 45 \
  ...
  echo "    tools/compile-check.sh                     (type-checks the Android sources)"
  exit 0
fi
...
[ -d "$SDK/build-tools/35.0.0" ]     || missing="$missing $BUILD_TOOLS"
