...
object AccessibilityMatrix {
    ...
    val pages: List<PagePass> = listOf(
        ...
        PagePass(
            ...
            states = requiredStates,
            voltAction = "Log set",
            talkBackNotes = "TalkBack order is header, compact exercise identity, set context, weight, reps, effort, suggestion, latest saved set, companion, commit. Exercise pictures remain decorative. Switch exercise is explicit; Session summary separates elapsed time and session totals from exercise progress. Weight, Added weight, and Assistance retain their meanings; zero external load is not called bodyweight. Numeric fields expose Decrease, Increase, and Type actions and select the current value when opened. Working/Warm-up and RPE have radio semantics. RPE help is always available; Clear or the selected choice removes effort. Warm-up presets are value-application buttons; suggestions are supporting text. Saving a warm-up returns to Working. View sets opens labeled working and warm-up rows with edit/delete menus. Ordinary saves retain the entry position; edits deliberately reveal entry. Commit names its payload and disabled reason. A saved receipt is announced once, while timer ticks remain silent. Time this set and Start rest are distinct named actions. Error/undo shares the companion with access to an active clock. Undo honors the accessibility timeout. Font 2.0 reflows choices and values. Reduced motion snaps geometry; dwell and announcements stay.",
            automatedEvidence = true,
        ),
        PagePass(
            id = "active-cardio",
