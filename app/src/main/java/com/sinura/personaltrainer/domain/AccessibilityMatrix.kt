...
object AccessibilityMatrix {
    ...
    val pages: List<PagePass> = listOf(
        ...
        PagePass(
            ...
            states = requiredStates,
            voltAction = "Log set",
            talkBackNotes = "TalkBack order is header, hero identity, set context, coach, weight, reps, RPE, recent sets, timer, context rail, Log. Rest is not announced every second. Idle rest says it is not running. Idle clock, Time this set, and Start rest sit in the timer row; duration presets live in a sheet. First rest names unrestricted battery. Rest Start/Skip and Log set sit in the lower dock. Process restore keeps the set. RPE chips are a radio row and name the meaning: RPE 8, about two reps left, not selected. Warm-up is outside that row. A selected warm-up ramp names WU and the weight. Set context speaks SET 3 OF 4, WU 2, or EXTRA 1. Weight and reps wells expose Decrease, Increase, and Type actions; the weight well is named Weight, Added, or Assistance for the lift, and 0 is no weight / bodyweight not 0 lbs. Log names its payload and its disabled reason. The success receipt and the rest completion speak once, politely; the running clock never streams seconds and there is no verbose countdown. Hero pictures stay decorative inside the named identity. Set rows expose Actions for set N. The undo offer stretches past 6 seconds under TalkBack and lives in the context rail. Font 2.0 wraps the coach and puts the RPE label above the chips. Reduced motion snaps geometry; dwell and announcements stay.",
            automatedEvidence = true,
        ),
        PagePass(
            id = "active-cardio",
