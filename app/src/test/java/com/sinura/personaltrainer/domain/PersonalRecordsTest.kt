...
class PersonalRecordsTest {
    ...
    fun bestsPickTheHeaviestSetForWeight() {
        ...
        val best = PersonalRecords.bests(history).getValue(PersonalRecordKind.WEIGHT)
    ...
    fun repsRecordIsMeasuredAtTheTopWeight() {
        ...
        val best = PersonalRecords.bests(history).getValue(PersonalRecordKind.REPS_AT_WEIGHT)
    ...
    fun emptyHistoryHasNoRecords() {
        assertTrue(PersonalRecords.bests(emptyList()).isEmpty())
    }

    @Test
    fun theFirstSetOfALiftSetsNoRecords() {
        assertEquals(emptySet<PersonalRecordKind>(), PersonalRecords.detect(rec(100.0, 5, 1), emptyList()))
    }

    @Test
    fun equallingARecordIsNotBreakingIt() {
        val prior = listOf(rec(100.0, 5, 1))
        assertEquals(emptySet<PersonalRecordKind>(), PersonalRecords.detect(rec(100.0, 5, 2), prior))
    }

    @Test
    fun aHeavierSetBreaksTheWeightRecord() {
        val prior = listOf(rec(100.0, 5, 1))
        val broken = PersonalRecords.detect(rec(102.5, 5, 2), prior)
    ...
    fun moreRepsAtTheSameWeightBreaksOnlyTheRepRecord() {
        val prior = listOf(rec(100.0, 5, 1))
        val broken = PersonalRecords.detect(rec(100.0, 6, 2), prior)
    ...
    fun moreRepsAtANewWeightIsNotARepRecord() {
        ...
        val prior = listOf(rec(100.0, 5, 1))
        val broken = PersonalRecords.detect(rec(105.0, 8, 2), prior)
        assertTrue(PersonalRecordKind.REPS_AT_WEIGHT !in broken)
        assertTrue(PersonalRecordKind.WEIGHT in broken)
    }
    ...
    fun aBackOffSetBreaksNothing() {
        ...
        assertEquals(emptySet<PersonalRecordKind>(), PersonalRecords.detect(rec(70.0, 8, 3), prior))
    }

    @Test
    fun aHighRepSetCannotBreakTheEstimateRecord() {
        val prior = listOf(rec(100.0, 5, 1))
        val broken = PersonalRecords.detect(rec(60.0, 30, 2), prior)
        assertTrue(PersonalRecordKind.ESTIMATED_ONE_REP_MAX !in broken)
    }
}
